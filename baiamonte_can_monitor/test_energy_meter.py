...
class EnergyMeterTests(unittest.TestCase):
    ...
    def test_persists_totals(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(json.dumps({
                "sign_convention": EnergyMeter.SIGN_CONVENTION,
                "charged_kwh": 12.5,
                "discharged_kwh": 8.25,
            }))
    ...
    def test_migrates_pre_061_energy_buckets_once(self):
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "energy.json"
            path.write_text(json.dumps({
                "charged_kwh": 12.5,
                "discharged_kwh": 8.25,
                "average_charge_w": 700,
                "average_discharge_w": 400,
            }))
            meter = EnergyMeter(path)
            self.assertEqual(meter.charged_kwh, 8.25)
            self.assertEqual(meter.discharged_kwh, 12.5)
            self.assertEqual(meter.average_charge_w, 400)
            self.assertEqual(meter.average_discharge_w, 700)
            meter.save()
            restored = EnergyMeter(path)
            self.assertEqual(restored.charged_kwh, 8.25)
            self.assertEqual(restored.discharged_kwh, 12.5)
