...
function renderRecovery(readings,data){
  ...
  const labels={normal:'Normal charging permitted',recovery:'Low-rate recovery required',charge_complete:'Charge complete — BMS tapering',charge_blocked:'Charging blocked for safety',monitoring_unavailable:'Monitoring incomplete',learning_limits:'Learning BMS limits'};
  ...
  $('recovery-badge').textContent=state==='normal'?'READY':state==='charge_complete'?'FULL':state==='charge_blocked'?'BLOCKED':'RECOVERY';
  $('recovery-badge').className=`badge ${state==='normal'||state==='charge_complete'?'neutral':'warning'}`;
  $('recovery-hero').className=`recovery-hero ${state}`;
  $('charge-safe').textContent=state==='charge_complete'?'CHARGE COMPLETE':safe?'CHARGE SAFE':'CHARGE LOCKED';
  $('charge-safe').className=`status-badge ${safe||state==='charge_complete'?'healthy':'error'}`;
  $('full-rate-state').textContent=state==='charge_complete'?'No more charge needed':full?'Full rate permitted':'Full rate locked';
  $('full-rate-detail').textContent=state==='charge_complete'?'Normal full-SOC BMS tapering; do not force current':full?'Permitted within BMS and wiring limits':'Locked until cells and pack SOC are balanced';
