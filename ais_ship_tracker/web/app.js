...
function renderMap(vessels,bounds){const map=$('#sea-map');$$('.vessel-marker').forEach(node=>node.remove());$('#map-empty').classList.toggle('hidden',vessels.length>0);const latSpan=Math.max(.0001,bounds.north-bounds.south),lonSpan=Math.max(.0001,bounds.east-bounds.west);vessels.forEach(v=>{if(typeof v.latitude!=='number'||typeof v.longitude!=='number')return;const x=Math.min(96,Math.max(4,(v.longitude-bounds.west)/lonSpan*100)),y=Math.min(94,Math.max(6,100-(v.latitude-bounds.south)/latSpan*100));const node=document.createElement('div');node.className='vessel-marker';node.style.left=`${x}%`;node.style.top=`${y}%`;node.dataset.name=v.name||'Unknown vessel';node.textContent='▲';node.title=`${v.name||'Unknown vessel'} · ${fmt(v.sog)} kn`;map.appendChild(node)})}
