...
let state=null,refreshRunning=false,refreshQueued=false,refreshFailures=0,selectedMapMmsi=null,activeMapArea=null,mapVesselsVisible=null;
let mapDisplayMode='labels';
let dashboardWeatherGeneration=0;
let dashboardWeatherMetadata=null,dashboardWeatherFetched=0;
let marineStreamUrl=null,marineAudioStopping=false;
...
function setMapArea(areaId){activeMapArea=areaId;selectedMapMmsi=null;resetOverviewMap();try{localStorage.setItem('baiamonteAisArea',areaId)}catch(error){}if(state)render(state);refresh()}
...
function renderDashboardTiles(view,style){const layer=$('#dashboard-tiles'),signature=[style,view.zoom,Math.round(view.left),Math.round(view.top),Math.round(view.width),Math.round(view.height)].join(':');if(layer.dataset.signature===signature)return;layer.dataset.signature=signature;const count=2**view.zoom;layer.innerHTML='';for(let y=Math.max(0,Math.floor(view.top/MAP_TILE));y<=Math.min(count-1,Math.floor((view.top+view.height)/MAP_TILE));y++){for(let x=Math.floor(view.left/MAP_TILE);x<=Math.floor((view.left+view.width)/MAP_TILE);x++){const image=document.createElement('img');image.alt='';image.decoding='async';image.src=`api/map-tile/${style}/${view.zoom}/${((x%count)+count)%count}/${y}.png`;image.style.left=`${x*MAP_TILE-view.left}px`;image.style.top=`${y*MAP_TILE-view.top}px`;layer.appendChild(image)}}}
async function getDashboardWeatherMetadata(){if(dashboardWeatherMetadata&&Date.now()-dashboardWeatherFetched<300000)return dashboardWeatherMetadata;const response=await fetch('api/weather-maps',{cache:'no-store'});if(!response.ok)throw new Error(response.status);dashboardWeatherMetadata=await response.json();dashboardWeatherFetched=Date.now();return dashboardWeatherMetadata}
async function renderDashboardWeather(view,cfg){
  ...
  if(!cfg.weather_overlay_dashboard){layer.innerHTML='';delete layer.dataset.signature;status.hidden=true;return}
  try{
    const metadata=await getDashboardWeatherMetadata();if(generation!==dashboardWeatherGeneration)return;
    ...
    const signature=[frame.path,zoom,Math.round(view.left),Math.round(view.top),Math.round(view.width),Math.round(view.height),cfg.tv_weather_opacity||65].join(':');
    if(layer.dataset.signature===signature){status.hidden=false;return}layer.dataset.signature=signature;layer.innerHTML='';
...
function renderMap(vessels,bounds,cfg){
  ...
  renderDashboardTiles(view,cfg.map_style||'standard');renderDashboardWeather(view,cfg);
  const existingMarkers=new Map($$('.vessel-marker').map(node=>[node.dataset.mmsi,node])),retainedMarkers=new Set();let positioned=0,selected=null;
  if(mapVesselsVisible)vessels.forEach(v=>{
    ...
    positioned+=1;const info=vesselFlag(v.mmsi),key=String(v.mmsi),node=existingMarkers.get(key)||document.createElement('button');if(!existingMarkers.has(key)){node.type='button';node.className='vessel-marker label-east';node.dataset.mmsi=key;world.appendChild(node)}retainedMarkers.add(key);node.classList.toggle('label-hidden',positioned>12);node.dataset.mapX=String(x);node.dataset.mapY=String(y);node.style.left=`${x}px`;node.style.top=`${y}px`;node.style.setProperty('--vessel-color',overviewVesselColor(v));node.title=`${info.country} · ${v.name||'Unknown vessel'} · ${fmt(v.sog)} kn`;
    const contentSignature=[v.name,v.vessel_type,v.vessel_class,v.sog,v.destination,v.last_seen,v.heading,v.cog].join('|');if(node.dataset.contentSignature!==contentSignature){node.dataset.contentSignature=contentSignature;node.innerHTML=`<span class="vessel-symbol" style="transform:rotate(${Number(v.heading??v.cog)||0}deg)">${overviewVesselIcon(v)}</span><span class="map-label-leader"></span><span class="map-vessel-label"><b>${info.flag} ${esc(v.name||'Unknown vessel')}</b><span>MMSI ${esc(v.mmsi)} · ${esc(v.vessel_type||v.vessel_class||'AIS contact')}</span><span>${esc(fmt(v.sog))} kn · ${esc(v.destination||'Destination not broadcast')}</span><em>${esc(ago(v.last_seen))}</em></span>`}node.onclick=event=>{event.stopPropagation();showMapVessel(v)};node.classList.toggle('selected',key===selectedMapMmsi);if(key===selectedMapMmsi)selected=v
  });
  existingMarkers.forEach((node,key)=>{if(!retainedMarkers.has(key))node.remove()});
...
async function startMarineAudio(){
  const player=$('#marine-player');if(!marineStreamUrl)return;
  marineAudioStopping=true;player.pause();player.src=`${marineStreamUrl}?listen=${Date.now()}`;player.load();marineAudioStopping=false;
  $('#marine-listen-status').textContent='Connecting…';$('#marine-listen-start').disabled=true;$('#marine-listen-stop').disabled=false;
  try{await player.play()}
  catch(error){stopMarineAudio('Playback blocked — press Start listening again.');console.error(error)}
}
function stopMarineAudio(message='Stopped'){
  const player=$('#marine-player');if(!player)return;
  marineAudioStopping=true;player.pause();player.removeAttribute('src');player.load();
  $('#marine-listen-status').textContent=message;$('#marine-listen-start').disabled=!marineStreamUrl;$('#marine-listen-stop').disabled=true;
  requestAnimationFrame(()=>{marineAudioStopping=false});
}
...
function renderMarineRadio(radio){
  ...
  marineStreamUrl=ready&&radio.stream_url?radio.stream_url:null;
  $('#marine-listen-start').disabled=!marineStreamUrl||Boolean(player.getAttribute('src'));
  $('#marine-listen-stop').disabled=!player.getAttribute('src');
  if(!marineStreamUrl&&player.getAttribute('src'))stopMarineAudio(radio.state==='Device conflict'?'Stopped — radio device conflict.':'Stopped — receiver stream unavailable.');
  $('#marine-player-help').textContent=ready?'Use Start listening to connect. Pause, Stop listening, leaving this page, or hiding the app completely closes the live audio connection.':radio.error||(!enabled?'Enable Marine VHF after connecting the second Nooelec.':'Starting the second receiver and private audio stream…');
...
function render(data){
  ...
  if($('#fleet').classList.contains('active'))$('#fleet-grid').innerHTML=vessels.length?vessels.map(vesselCard).join(''):'<article class="panel empty">No positioned AIS broadcasts have arrived in this watch area yet.</article>';
...
async function refresh(){if(refreshRunning){refreshQueued=true;return}refreshRunning=true;const requestedArea=activeMapArea,button=$('#refresh');button.disabled=true;try{const areaQuery=requestedArea?`?area=${encodeURIComponent(requestedArea)}`:'';const response=await fetch(`api/status${areaQuery}`,{cache:'no-store'});if(!response.ok)throw new Error(`Dashboard API ${response.status}`);const data=await response.json();refreshFailures=0;if(!requestedArea||requestedArea===activeMapArea)render(data)}catch(error){refreshFailures+=1;$('#last-check').textContent=state?'Update delayed · retrying':'Connecting to dashboard…';if(!state&&refreshFailures>=3){$('#receiver-status').textContent='Dashboard unavailable';$('#hero-status').textContent='Waiting for the Baiamonte AIS service'}console.error(error)}finally{refreshRunning=false;button.disabled=false;if(refreshQueued){refreshQueued=false;refresh()}}}
$('#refresh').onclick=refresh;$('#marine-recover').onclick=recoverMarineRadio;$('#marine-listen-start').onclick=startMarineAudio;$('#marine-listen-stop').onclick=()=>stopMarineAudio();$('#marine-player').addEventListener('playing',()=>{$('#marine-listen-status').textContent='Listening';$('#marine-listen-start').disabled=true;$('#marine-listen-stop').disabled=false});$('#marine-player').addEventListener('waiting',()=>{if(!marineAudioStopping&&$('#marine-player').getAttribute('src'))$('#marine-listen-status').textContent='Buffering…'});$('#marine-player').addEventListener('stalled',()=>{if(!marineAudioStopping&&$('#marine-player').getAttribute('src'))$('#marine-listen-status').textContent='Reconnecting audio…'});$('#marine-player').addEventListener('error',()=>{if(!marineAudioStopping&&$('#marine-player').getAttribute('src'))stopMarineAudio('Audio connection failed — try Start listening again.')});showPage(location.hash.slice(1)||'overview');refresh();setInterval(()=>{if(!document.hidden)refresh()},10000);document.addEventListener('visibilitychange',()=>{if(document.hidden)stopMarineAudio('Stopped while the app is hidden.');else{resetOverviewMap();rerenderVisibleMap();refresh()}});addEventListener('pagehide',()=>stopMarineAudio());addEventListener('beforeunload',()=>stopMarineAudio());addEventListener('resize',rerenderVisibleMap);
