...
let state=null,refreshRunning=false,refreshFailures=0;
...
function renderMap(vessels,bounds,cfg){if(!mapIsVisible())return;const world=$('#map-world'),view=dashboardView(bounds);renderDashboardTiles(view,cfg.map_style||'standard');renderDashboardWeather(view,cfg);$$('.vessel-marker').forEach(node=>node.remove());let positioned=0;vessels.forEach(v=>{if(typeof v.latitude!=='number'||typeof v.longitude!=='number')return;const point=geoPoint(v.latitude,v.longitude,view.zoom),x=point.x-view.left,y=point.y-view.top;if(x<0||y<0||x>view.width||y>view.height)return;positioned+=1;const info=vesselFlag(v.mmsi),node=document.createElement('div');node.className='vessel-marker';node.style.left=`${x}px`;node.style.top=`${y}px`;node.dataset.name=`${info.flag} ${v.name||'Unknown vessel'}`;node.textContent='▲';node.title=`${info.country} · ${v.name||'Unknown vessel'} · ${fmt(v.sog)} kn`;world.appendChild(node)});$('#map-empty').classList.toggle('hidden',positioned>0)}
