...
function showPage(id){$$('.nav').forEach(node=>node.classList.toggle('active',node.dataset.page===id));$$('.page').forEach(node=>node.classList.toggle('active',node.id===id));$('#page-title').textContent={overview:'Overview',fleet:'Live traffic',area:'Watch area'}[id]||'Overview';history.replaceState(null,'',`#${id}`);window.scrollTo({top:0,behavior:'smooth'})}
$$('.nav').forEach(node=>node.onclick=()=>showPage(node.dataset.page));$$('[data-go]').forEach(node=>node.onclick=()=>showPage(node.dataset.go));

const MAP_TILE=256;
const midInfo={202:['AD','Andorra'],211:['DE','Germany'],215:['MT','Malta'],218:['DE','Germany'],224:['ES','Spain'],225:['ES','Spain'],226:['FR','France'],227:['FR','France'],228:['FR','France'],229:['MT','Malta'],232:['GB','United Kingdom'],233:['GB','United Kingdom'],234:['GB','United Kingdom'],235:['GB','United Kingdom'],237:['GR','Greece'],239:['GR','Greece'],240:['GR','Greece'],241:['GR','Greece'],247:['IT','Italy'],248:['MT','Malta'],249:['MT','Malta'],256:['MT','Malta'],257:['NO','Norway'],258:['NO','Norway'],259:['NO','Norway'],370:['PA','Panama'],371:['PA','Panama'],372:['PA','Panama'],373:['PA','Panama'],374:['PA','Panama'],538:['MH','Marshall Islands'],636:['LR','Liberia']};
function vesselFlag(mmsi){const item=midInfo[Number(String(mmsi||'').slice(0,3))];if(!item)return{flag:'🏳',country:'Unknown flag'};return{flag:[...item[0]].map(char=>String.fromCodePoint(127397+char.charCodeAt())).join(''),country:item[1]}}
...
function vesselCard(v){const info=vesselFlag(v.mmsi),mark=(v.call_sign||v.imo_number||'AIS').toString().slice(0,8);return`<article class="vessel-card"><div class="vessel-card-top"><div class="identity-mark"><span class="card-flag" title="${esc(info.country)}">${info.flag}</span><span class="operator-mark">${esc(mark)}</span></div><span class="status">${esc(v.nav_status_string||'Not defined')}</span></div><h3>${esc(v.name||'Unknown vessel')}</h3><span class="mmsi">${esc(info.country)} · MMSI ${esc(v.mmsi)} · ${esc(v.vessel_type||v.vessel_class||'Unknown type')}</span><div class="vessel-metrics"><div><small>SPEED</small><b>${esc(fmt(v.sog))} kn</b></div><div><small>COURSE</small><b>${esc(fmt(v.cog))}°</b></div><div><small>HEADING</small><b>${esc(fmt(v.heading))}°</b></div><div><small>SEEN</small><b>${esc(ago(v.last_seen))}</b></div></div><div class="destination"><span>Destination</span><b>${esc(v.destination||'Not broadcast')}</b></div></article>`}
