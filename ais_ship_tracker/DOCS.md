...
For the full Miami and Sicily map datasets, enable **Rahamin AIS Private Proxy**. The default endpoint is `http://192.168.86.196:8999/api/status` on the private routed network. Baiamonte AIS requests the Pi's separate `?area=miami` and `?area=baiamonte` caches and imports each only into its matching map. The raw UDP stream remains useful as the direct Miami radio path, while the private status proxy supplies the broader cached vessel views. While this proxy is enabled, Home Assistant does not make duplicate direct AISHub requests; the Miami Pi remains the single reciprocal API client.
