...
For the full Miami and Sicily map datasets, keep **AIS Network Data Source** set to **Rahamin Miami single-key proxy**. The default endpoint is `http://192.168.86.196:8999/api/status` on the private routed network. Baiamonte AIS requests the Pi's separate `?area=miami` and `?area=baiamonte` caches and imports each only into its matching map. The source area's scope is authoritative, so a harmless difference between the two installations' local bounds cannot silently remove Rahamin vessels. The raw UDP stream remains useful as the direct Miami radio path, while the private status proxy supplies the broader cached vessel views. In this mode Home Assistant never polls AISHub directly, even if an older username remains stored; the Miami Pi is the sole reciprocal API client.
